...
def main():
    ...
    df["created_at"] = df["created_at"].apply(lambda x: try_parse_date(x))
    df["created_at"] = df["created_at"].apply(lambda dt: dt.isoformat() if pd.notnull(dt) and str(dt) != "NaT" else "")
